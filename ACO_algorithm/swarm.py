...
class Swarm:
    def __init__(self, graph, ant_amount, starter_node, path_iteration, delta, decay):
        self.delta=delta
        self.decay=decay
        self.graph=graph
    ...
    def update_pheromones(self, ant):  #this function is meant to be executed once self.updated_pheromone_path is empty
        ...
        for node_index, node in enumerate(ant.path[0][:-1]):
            ant.graph.graph_pheromone[ant.path[0][node_index + 1]][ant.path[0][node_index]] += ant.delta / path_size
            ant.graph.graph_pheromone[ant.path[0][node_index]][ant.path[0][node_index + 1]] += ant.delta / path_size


    def generate_ant_list(self, starter_node, ant_amount):
        self.ant_list=[Ant(starter_node, self.graph, starter_node, self.delta, self.decay) for x in range(ant_amount)]
